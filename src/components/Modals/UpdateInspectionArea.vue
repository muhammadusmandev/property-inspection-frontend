        ...
        <CModalBody>
            <CForm class="row g-2 mt-0" @submit.prevent="handleUpdateArea">
                <CCol xs="12">
                    ...
                    <p class="text-body-secondary text-center my-1 mb-4 w-75 mx-auto">Update inspection area with custom name</p>
                ...
                <div class="d-grid mt-3 mb-3">
                    <CButton color="primary" class="px-4 py-2 self-button w-75 mx-auto mt-1" type="submit" :disabled="btnLoading"> <CIcon icon="cilUserPlus" v-if="!btnLoading" /> <ButtonSpinner v-if="btnLoading" size="small" bgColor="#000000" /> {{ btnLoading ? 'Updating...' : 'Update Area' }}</CButton>
...
<script setup>
    ...
    const { data: itemsList, execute: execute1 } = useApi(getInspectionAreaItemsList, false)
    const { loading: btnLoading, execute: execute2 } = useApi(updateInspectionArea, false)
    ...
    function setupPageFieldsData(){
        const selectedItemsIds = areaData.value.items.map(item => item.id)
        
        // set form data
        setFieldValue('name', areaData.value.name)
        setFieldValue('items', selectedItemsIds)
    }
    ...
    const handleUpdateArea = handleSubmit(async (formData) => {
        const response = await execute2({ pathParams: [props.areaId], payload: formData })
        ...
        if(response.success === true){
            ...
            showToast('error', 'Oops! Something went wrong. Failed to update area.')
        }
    })
