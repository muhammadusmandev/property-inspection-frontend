  ...
  <div class="datatable-container">
      <div class="datatable-header-top row align-items-center justify-between">
          <div class="page-heading col-md-6">
            <h2>Properties</h2>
            <p>Full control, monitor and manage all your properties in one place</p>
          </div>
          <div class="text-end pb-4 col-md-6 btns-row">
            <CButton class="px-4 self-bg-primary self-color-tertiary fs-8" as="a" href="/inspector/properties/add_property">
              <CIcon icon="cil-plus" /> Add New Property
            </CButton>
      ...
      <DataTable 
        ...
        >
          ...
          <Column header="Client" style="height: 44px">
            <template #body="{ data }">
              {{ data.client?.name ?? '-' }}
            </template>
          </Column>
          ...
          <Column header="Action" style="height: 44px">
            <template #body="{ data }">
              <div class="d-flex gap-1">
                <CButton class="badge bg-dark" as="a" :href="`/inspector/properties/property/${data.id}`"><CIcon icon="cil-pen" /></CButton>              
              </div>
            </template>
          </Column>
      </DataTable>
...
<script setup>
  ...
  const { loading, data, execute } = useApi(getProperties, false)
