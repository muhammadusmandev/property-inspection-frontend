<template>
    ...
    >
        <CModalBody>
            <CForm class="row g-2 mt-0" @submit.prevent="handleUpdateArea">
                <CCol xs="12">
                    <h2 class="mt-2 fw-bold mb-0 text-center">Update Inspection Area</h2>
                    <p class="text-body-secondary text-center my-1 mb-4 w-75 mx-auto">Update inspection area details</p>
...
<script setup>
    ...
    const schema = toTypedSchema( yup.object({
        name: yup.string().max(150, 'Max 150 characters long').required('Name is required'),
        condition: yup.string().required('Condition is required'),
        cleanliness: yup.string().required('Cleanliness is required'),
        description: yup.string(),
        items: yup.array().min(1).required('Must add at least one item')
    }))

    const { handleSubmit, setFieldValue } = useForm({ validationSchema: schema })

    const { value: name, errorMessage: nameError, validate: nameValidate, meta: nameMeta } = useField('name')
    const { value: condition, errorMessage: conditionError, validate: conditionValidate, meta: conditionMeta } = useField('condition')
    const { value: cleanliness, errorMessage: cleanlinessError, validate: cleanlinessValidate, meta: cleanlinessMeta } = useField('cleanliness')
    const { value: description, errorMessage: descriptionError, validate: descriptionValidate, meta: descriptionMeta } = useField('description')
    const { value: items, errorMessage: itemsError, validate: itemsValidate, meta: itemsMeta } = useField('items')

    const emit = defineEmits(['update:visibility'])

    watch(() => props.areaId, async (newId) => {
        if(newId && newId > 0) {
            await execute({ pathParams: [props.areaId] })
            setupPageFieldsData()
        }
    })
    ...
    const handleUpdateArea = handleSubmit(async (formData) => {
        const response = await executeUpdate({ pathParams: [props.areaId], payload: formData })
        if(response.success) {
            showToast('success', 'Area updated successfully! Wait redirecting...')
