  ...
  <div class="datatable-container">
      <div class="datatable-header-top row align-items-center justify-between">
          ...
          <div class="text-end pb-4 col-md-6 btns-row">
            <CButton class="px-4 self-bg-primary self-color-tertiary fs-8" @click="visibleAddReport()">
        ...
        <CForm class="row align-items-center justify-between" @submit.prevent="searchByColumnName">
          ...
          <div class="text-end col-md-3 search-btn">
            <CButton class="px-4 self-bg-light-dark self-color-tertiary fs-8" type="submit">
              <CIcon icon="cil-zoom" v-if="!btnLoading1" /> <ButtonSpinner v-if="btnLoading1" size="small" bgColor="#000000" /> {{ btnLoading1 ? 'Searching...' : 'Search Report' }} 
            </CButton>
          </div>
        </CForm>
      </div>
...
<script setup>
  ...
  const { loading, data, execute } = useApi(getReports, false)
  const { execute: execute1 } = useApi(deleteReport, false)

  onBeforeMount(async () => {
    await loadLazyReports({ first: 0, rows: perPage.value })
  })

  async function loadLazyReports(event, queryParams = {}){
  ...
  function handleShowUpdate(reportId) {
    router.push({ name: 'inspector.reports.view', params: { id: reportId } })
  }
