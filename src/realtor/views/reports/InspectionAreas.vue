<template>
  <CRow v-if="!reportData?.locked_at">
    <div class="d-flex justify-content-between align-items-center mb-4 mt-4 p-4" style="border: 1px solid #e0e0e0">
      <div>
        <h3>
          <CIcon icon="cil-room" size="xxl" /> Inspection Areas
        </h3>
        <p class="text-secondary fs-7">Manage selected template areas and custom inspection areas</p>
      </div>
      <div class="my-3 text-end">
        <CButton class="px-3 py-2 bg-dark text-white" style="border-radius: 25px; font-size: 13px;"
          @click="visibleAddNewArea()">
    ...
    <CAccordion class="px-0" v-if="reportData?.areas.length > 0">
      <!-- Area -->
      <CAccordionItem v-for="area in reportData.areas" :key="area.id" :item-title="area.name" class="mb-3">
        <div class="d-flex justify-content-between align-items-center mb-3">
          <h5 class="mb-0">{{ area.name }}</h5>

          <div class="d-flex gap-2 align-items-center">
            <CButton class="px-3 py-1 h-100 text-white"
              v-if="refAreaIdForImages === area.id && selectedImages.length > 0"
              style="border-radius: 25px; cursor: pointer; font-size: 0.78rem" color="info"
              @click="uploadInpsectionAreaPhotos" :disabled="uploadBtnLoading">
              <CIcon icon="cil-cloud-upload" v-if="!uploadBtnLoading" />
              <ButtonSpinner v-if="uploadBtnLoading" size="small" bgColor="#000000" /> {{ uploadBtnLoading ?
              'Uploading...' : 'Upload ' + selectedImages.length + ' Selected Photos' }}
            </CButton>
            <CButton class="px-3 py-1 h-100 text-white" v-else
              style="border-radius: 25px; cursor: pointer; font-size: 0.78rem" color="info"
              @click="visibleShowImagesUpload(area.id)">
        ...
        <CTable hover responsive class="mb-3">
          <CTableHead>
            <CTableRow>
              <CTableHeaderCell style="width: 10%;">Area Quality</CTableHeaderCell>
          ...
          <CTableBody>
            <CTableRow>
              <CTableDataCell style="width: 10%;">
                <CBadge class="text-capitalize px-3 py-2" style="border-radius: 25px; font-weight: 300"
                  :color="getStatusColor(area.condition)">
                  Condition - {{ area.condition }}
                </CBadge>
                <CBadge class="text-capitalize px-3 py-2 mt-1" style="border-radius: 25px; font-weight: 300"
                  :color="getStatusColor(area.condition)">
                  Cleanliness - {{ area.cleanliness }}
                </CBadge>
              </CTableDataCell>
              <CTableDataCell style="width: 25%;" class="fs-8 text-secondary">{{ area.description || '-' }}
              </CTableDataCell>
              <CTableDataCell style="width: 30%;" class="fs-8 text-secondary">
                <div class="d-flex flex-wrap gap-2" v-if="area.media.length">
                  <div class="position-relative" v-for="(image, index) in area.media">
                    <CImage class="area-img"
                      :src="createServerImageURL(image.thumbnail_path ? image.thumbnail_path : image.file_path)"
                      alt="Area Image" width="50" height="59" />
                    <button class="remove-selected-img bg-dark text-white border-0 position-absolute"
                      @click="deleteImg(image.id, 'area')">×</button>
                  </div>
                </div>
                <p class="fs-8" v-else>
        ...
        <CTable v-if="area.defects?.length > 0" hover responsive>
          ...
          <CTableBody class="fs-8">
            <CTableRow v-for="(defect, idx) in area.defects">
              ...
              <CTableDataCell style="width: 10%;" class="text-secondary">{{ defect.area_item_name || '-' }}
              </CTableDataCell>
              ...
              <CTableDataCell style="width: 25%;">
                <div class="d-flex flex-wrap gap-2" v-if="defect.images?.length">
                  <div class="position-relative" v-for="(image, index) in defect.images">
                    <CImage class="area-img"
                      :src="createServerImageURL(image.thumbnail_path ? image.thumbnail_path : image.file_path)"
                      alt="Defect Image" width="50" height="59" />
                    <button class="remove-selected-img bg-dark text-white border-0 position-absolute"
                      @click="deleteImg(image.id, 'defect')">×</button>
                  </div>
                </div>
                <p v-else>
                  No Image Uploaded Yet
              ...
              <CTableDataCell style="width: 10%;" class="fs-8 text-secondary">
                <span class="badge bg-dark" style="cursor: pointer;" @click="handleShowDefectUpdateModal(defect.id)">
                  <CIcon icon="cil-pen" />
                </span><br>
                <span class="badge bg-danger mt-1" style="cursor: pointer;"
                  @click="handleShowDeleteModal(defect.id, 'defect')">
                  <CIcon icon="cil-x" />
                </span>
              </CTableDataCell>
            </CTableRow>
        ...
        <div class="d-block text-center">
          <CButton color="primary" class="px-4 py-1 self-button mx-auto d-inline-block fs-8"
            @click="visibleAddAreaDefects(area.id)">
            <CIcon icon="cil-warning" /> Add Defect
          </CButton>
        </div>
        <!-- Defects -->
      </CAccordionItem>
  ...
  <DeleteWarningModal v-model:visibility="showDeleteModal" :btnLoading="deleteBtnLoading"
    @confirmedDelete="handleDeleteResource" />
  <MultipleImagesSelector v-model:visibility="showImagesUpload" @images-selected="onImagesSelected"
    :refItemId="refAreaIdForImages" :resetImages="resetImages" />
</template>
